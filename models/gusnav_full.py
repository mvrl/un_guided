...
class gusnav(nn.Module):    # HiDT with distributions
    def __init__(self, latent_dim):
        super(gusnav, self).__init__()
        ...
        self.style_encoder = StyleEncoder(num_downsamples=4, input_dim=3, output_dim=self.latent_dim, dim=48,
        ...
        if cfg.data.image_size[0]==256:
            self.unguided_mlp = MLP_Distribution(512+self.latent_dim, self.latent_dim) # 256x256
        elif cfg.data.image_size[0]==320:
            self.unguided_mlp = MLP_Distribution(800+self.latent_dim, self.latent_dim) # 320x320
        else:
            self.unguided_mlp = MLP_Distribution(2048+self.latent_dim, self.latent_dim)   #  512x512
        ...
        self.decoder = DecoderUnet(num_upsamples=2, num_blocks=5, dim=192, res_norm='adain', activ='relu',
                     ...
                     adain_net={'architecture':'MLP', 'input_dim':self.latent_dim, 'dim':64, 'num_blocks':3,})
    ...
    def forward(self, source, style=None, training=True):
        ...
        source_features = torch.cat( [source_features, source_style.squeeze(3).squeeze(2)], dim=1 )
        ...
        if style is not None:
            ...
            # guided distribution
            mu_guided, log_var_guided = self.guided_mlp(target_style.squeeze(3).squeeze(2).view                                                        (source.shape[0],-1),return_everything=True)
        ...
        if training:
            ...
            return prediction, unguided_distribution.mean, unguided_distribution.stddev,  guided_distribution.mean, guided_distribution.stddev, feedback_vector  # return everything during training
